...
const VehicleDetails = () => {
  ...
  const handleNext = async () => {
    ...
    if (!hasFrontVisual) {
      ErrorToast('Please upload front image');
      return;
    }
    ...
    try {
      ...
      if (!fFront && frontImagePreview && /^https?:\/\//i.test(String(frontImagePreview))) {
        fFront = await fetchUrlAsFile(frontImagePreview, 'vehicle-reg-front');
      }
      if (!fFront && frontImagePreview && String(frontImagePreview).startsWith('data:')) {
        const res = await fetch(frontImagePreview);
        const blob = await res.blob();
        fFront = new File([blob], 'vehicle-reg-front.jpg', { type: blob.type || 'image/jpeg' });
      }
      if (!fFront) {
        ErrorToast('Could not load front image. Please upload again.');
        return;
      }
