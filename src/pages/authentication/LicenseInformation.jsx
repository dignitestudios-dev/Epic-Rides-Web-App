...
const LicenseInformation = () => {
  ...
  const validateField = (name, value) => {
    ...
    if (name === 'licenseNumber') {
      ...
      } else if (!LICENSE_NUMBER_REGEX.test(value.trim())) {
        error = 'License number must be 6-15 uppercase letters or numbers';
      }
  ...
  const handleLicenseInputChange = (e) => {
    ...
    // Only allow uppercase letters/numbers for license number and limit to 15 chars
    if (name === 'licenseNumber') {
  ...
  const handleNext = async () => {
    ...
    const hasFrontVisual =
      Boolean(frontImage) ||
      (frontImagePreview &&
        (String(frontImagePreview).startsWith('data:') ||
          /^https?:\/\//i.test(String(frontImagePreview))));
    const hasBackVisual =
      Boolean(backImage) ||
      (backImagePreview &&
        (String(backImagePreview).startsWith('data:') ||
          /^https?:\/\//i.test(String(backImagePreview))));
    ...
    try {
      let fFront = frontImage;
      let fBack = backImage;
      if (!fFront && frontImagePreview && /^https?:\/\//i.test(String(frontImagePreview))) {
        fFront = await fetchUrlAsFile(frontImagePreview, 'license-front');
      }
      if (!fFront && frontImagePreview && String(frontImagePreview).startsWith('data:')) {
        const res = await fetch(frontImagePreview);
        const blob = await res.blob();
        fFront = new File([blob], 'license-front.jpg', { type: blob.type || 'image/jpeg' });
      }
      if (!fBack && backImagePreview && /^https?:\/\//i.test(String(backImagePreview))) {
        fBack = await fetchUrlAsFile(backImagePreview, 'license-back');
      }
      if (!fBack && backImagePreview && String(backImagePreview).startsWith('data:')) {
        const res = await fetch(backImagePreview);
        const blob = await res.blob();
        fBack = new File([blob], 'license-back.jpg', { type: blob.type || 'image/jpeg' });
      }

      if (!fFront || !fBack) {
        setFieldErrors((prev) => ({
          ...prev,
          ...(!fFront ? { frontImage: 'Upload front image' } : {}),
          ...(!fBack ? { backImage: 'Upload back image' } : {}),
        }));
        ErrorToast('Please upload both sides of your license (or wait for previous images to load).');
        return;
      }

      const files = [fFront, fBack];
      ...
      // Recovery case: request succeeded on backend but client timed out/retried.
      const errorMessage = typeof error === 'string'
        ? error
        : error?.message || error?.error || '';
      const normalizedMessage = errorMessage.toLowerCase();
      const isAlreadySubmitted =
        normalizedMessage.includes('driverlicense already submitted') ||
  ...
  // Keep user on current onboarding step even on browser back.
  React.useEffect(() => {
    const handlePopState = () => {
      navigate('/license-information', { replace: true, state: location.state });
    };
            ...
            <div>
              ...
              <div className="relative w-full flex items-center">
                <input
                  ...
                  className="w-full px-3 pr-10 py-2.5 rounded-lg outline-none placeholder:text-[#808080]"
                  style={{
                    background: 'linear-gradient(180deg, rgba(97, 203, 8, 0.12) 0%, rgba(97, 203, 8, 0.04) 50%, rgba(97, 203, 8, 0.07) 100%)',
                    backdropFilter: 'blur(42px)',
