...
const SOCKET_URL = "https://api.dev.epicridesapp.com";
...
const RideTracking = ({ ride: rideProp }) => {
  ...
  useEffect(() => {
    ...
    socket.on("ride:status:update", (payload) => {
      ...
      if (!payload?.rideStatus) return;
      setLiveRide((prev) =>
        prev ? { ...prev, rideStatus: payload.rideStatus } : prev
      );
    });
    ...
    socket.on("driver:location:update", (payload) => {
      ...
      // ✅ Stop demo animation immediately when real GPS arrives
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Destroy old map instance and markers before re-initializing
    if (mapInstanceRef.current) {
      if (animationFrameRef.current) window.cancelAnimationFrame(animationFrameRef.current);
  ...
  // ✅ Fetch real road-following route using DirectionsService
  // Uses leg steps to extract ALL road waypoints (not just overview_path)
    ...
    directionsService.route(
      {
        ...
        // ✅ Request high-resolution path via step-level polylines
      ...
      (result, status) => {
        if (status === "OK" && result?.routes?.[0]) {
          ...
          // ✅ Extract detailed path from every step of every leg
          // This gives road-following points vs overview_path which is lossy
          const detailedPath = [];
          route.legs.forEach((leg) => {
            leg.steps.forEach((step) => {
              // Each step has its own encoded polyline with full road detail
              const stepPath = step.path || [];
              stepPath.forEach((latLng) => detailedPath.push(latLng));
            });
          });
          ...
          // Fallback to overview_path if steps had no detail
          const overviewPath = route.overview_path || [];
          if (overviewPath.length > 1) {
            console.log(`⚠️ Using overview_path: ${overviewPath.length} points`);
        ...
        console.warn("⚠️ Directions failed, using straight line. Status:", status);
        directionsPathRef.current = [];
        startCarAnimation();
      }
    );
  ...
  // Animate car along route (demo mode — stops when real GPS arrives)
  const startCarAnimation = () => {
    ...
    const animate = (timestamp) => {
      // Real GPS from socket arrived — stop demo animation completely
      ...
      const TRIP_DURATION_MS = 28000;
      tRef.current += delta / TRIP_DURATION_MS;
      if (tRef.current > 1) tRef.current -= 1;

      setProgress(Math.round(tRef.current * 100));

      let lat, lng, segmentFrom, segmentTo;

      if (path && path.length > 1) {
        const totalSegments = path.length - 1;
        const pathT = tRef.current * totalSegments;
        const idx = Math.min(totalSegments - 1, Math.floor(pathT));
        const segT = pathT - idx;
        const p1 = path[idx];
        const p2 = path[idx + 1];
        segmentFrom = p1;
        segmentTo = p2;
        ...
        const p1 = pickupLatLng;
        const p2 = dropoffLatLng;
        segmentFrom = new window.google.maps.LatLng(p1.lat, p1.lng);
        segmentTo = new window.google.maps.LatLng(p2.lat, p2.lng);
        lat = p1.lat + (p2.lat - p1.lat) * tRef.current;
        lng = p1.lng + (p2.lng - p1.lng) * tRef.current;
      } else {
        animationFrameRef.current = window.requestAnimationFrame(animate);
        return;
      }

      if (segmentFrom && segmentTo && window.google?.maps?.geometry) {
        const from = segmentFrom instanceof window.google.maps.LatLng
          ? segmentFrom : new window.google.maps.LatLng(segmentFrom.lat(), segmentFrom.lng());
        const to = segmentTo instanceof window.google.maps.LatLng
          ? segmentTo : new window.google.maps.LatLng(segmentTo.lat(), segmentTo.lng());
        const rawHeading = window.google.maps.geometry.spherical.computeHeading(from, to) || 0;
        const smoothed = headingRef.current + (rawHeading - headingRef.current) * 0.25;
        headingRef.current = smoothed;
        if (carElementRef.current) {
          carElementRef.current.style.transform = `translate(-50%, -50%) rotate(${smoothed}deg)`;
        }
      }
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col overflow-hidden">
      ...
      {/* Driver card */}
      <div className="absolute top-6 right-6 z-20 bg-black/60 backdrop-blur-lg border border-white/10 rounded-2xl p-4 w-64 shadow-2xl">
      ...
      <div className={`absolute bottom-0 left-0 right-0 z-20 bg-gradient-to-t from-black via-black to-black/90 border-t border-white/10 rounded-t-3xl transition-all duration-500 backdrop-blur-xl overflow-y-auto ${isExpanded ? "h-screen" : "h-fit max-h-96"}`}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white/5 rounded-xl p-3">
              ...
              <p className="text-lg font-bold text-white mt-2 flex items-center gap-1">
                ...
                {liveRide?.averageTime != null ? `${liveRide.averageTime} min` : "--"}
              </p>
            </div>
            <div className="bg-white/5 rounded-xl p-3">
              ...
              <p className="text-lg font-bold text-white mt-2 flex items-center gap-1">
                ...
                {liveRide?.rideDistance != null ? `${liveRide.rideDistance} km` : "--"}
              </p>
            </div>
            <div className="bg-white/5 rounded-xl p-3">
        ...
        {isExpanded && (
          <div className="px-6 py-4 space-y-3">
            <button className="w-full py-3 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-xl font-semibold text-sm transition-all border border-green-500/30 flex items-center justify-center gap-2">
              <Phone size={18} /> Call Driver
            </button>
            <button className="w-full py-3 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded-xl font-semibold text-sm transition-all border border-blue-500/30 flex items-center justify-center gap-2">
              <MessageCircle size={18} /> Send Message
            </button>
            <button className="w-full py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl font-semibold text-sm transition-all border border-red-500/30">
              Cancel Ride
            </button>
          </div>
        )}
